...
def main():
    try:
        ...
        # Calcular límites automáticamente desde namelist.wps
        # Esto nos permite eliminar las secciones [domain_bounds] del config.ini
        def get_wps_val(c, k, dt=float, d=None):
            m = re.search(r'(?i)\b'+k+r'\s*=\s*([^\n]+)', c)
            if not m: return d
            v = m.group(1).split('!')[0].split('/')[0]
            vs = [x.strip().strip("'").strip('"') for x in v.split(',') if x.strip()]
            try: return [dt(x) for x in vs]
            except: return d

        wps_path = Path(c_tmp['paths'].get('namelist_path', '')) / 'namelist.wps'
        if not wps_path.exists():
            # Try to find it in the run_dir
            run_dir = c_tmp['paths'].get('run_dir', '')
            if run_dir:
                wps_path = Path(run_dir) / 'namelist.wps'

        domain_bounds = {}
        if wps_path.exists():
            import pyproj
            import re
            c = open(wps_path, 'r').read()
            max_dom = get_wps_val(c, 'max_dom', int, [1])[0]
            map_proj = (re.search(r"(?i)map_proj\s*=\s*['\"](\w+)['\"]", c) or re.search(r"(?i)map_proj\s*=\s*(\w+)", c)).group(1).lower()
            ref_lat = get_wps_val(c, 'ref_lat', float)[0]
            ref_lon = get_wps_val(c, 'ref_lon', float)[0]
            truelat1 = get_wps_val(c, 'truelat1', float, [ref_lat])[0]
            truelat2 = get_wps_val(c, 'truelat2', float, [truelat1])[0]
            stand_lon = get_wps_val(c, 'stand_lon', float, [ref_lon])[0]
            e_we, e_sn = get_wps_val(c, 'e_we', int), get_wps_val(c, 'e_sn', int)
            dx = get_wps_val(c, 'dx', float)[0]
            dy = get_wps_val(c, 'dy', float, [dx])[0]
            i_p, j_p = get_wps_val(c, 'i_parent_start', int, [1]*max_dom), get_wps_val(c, 'j_parent_start', int, [1]*max_dom)
            p_r = get_wps_val(c, 'parent_grid_ratio', int, [1]*max_dom)
            
            WRF_R = 6370000
            if map_proj == 'lambert': p = pyproj.Proj(proj='lcc', lat_1=truelat1, lat_2=truelat2, lat_0=ref_lat, lon_0=stand_lon, a=WRF_R, b=WRF_R)
            elif map_proj == 'mercator': p = pyproj.Proj(proj='merc', lat_ts=truelat1, lon_0=stand_lon, a=WRF_R, b=WRF_R)
            elif map_proj == 'polar': p = pyproj.Proj(proj='stere', lat_ts=truelat1, lat_0=90.0 if truelat1 > 0 else -90.0, lon_0=stand_lon, a=WRF_R, b=WRF_R)
            else: p = None
            
            if p:
                cx, cy = p(ref_lon, ref_lat)
                ox, oy = [cx - (e_we[0]-1)/2.0 * dx], [cy - (e_sn[0]-1)/2.0 * dy]
                cdx, cdy = [dx], [dy]
                for i in range(1, max_dom):
                    cdx.append(cdx[i-1]/p_r[i]); cdy.append(cdy[i-1]/p_r[i])
                    ox.append(ox[i-1] + (i_p[i]-1)*cdx[i-1]); oy.append(oy[i-1] + (j_p[i]-1)*cdy[i-1])
                for i in range(max_dom):
                    w, h = (e_we[i]-1)*cdx[i], (e_sn[i]-1)*cdy[i]
                    lons, lats = p([ox[i], ox[i]+w], [oy[i], oy[i]+h], inverse=True)
                    domain_bounds[f"d{i+1:02d}"] = { "left": min(lons), "right": max(lons), "bottom": min(lats), "top": max(lats) }

        if domain_bounds:
            config_data["domain_bounds"] = domain_bounds
        else:
            config_data["domain_bounds"] = {}
