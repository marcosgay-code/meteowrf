...
def get_rain(ncfile, prevnc=None):
   ...
   if not prevnc is None:
      ...
      hourly_rain = rain - rain0
      
      # Si la lluvia por hora es negativa, significa que el acumulador de WRF se ha reseteado
      # al empezar una nueva tanda de modelado. En este caso, la precipitación de esta primera hora
      # es exactamente la acumulada puramente en 'rain'.
      if hasattr(hourly_rain, "values"):
          hourly_rain.values = np.where(hourly_rain.values < 0, rain.values, hourly_rain.values)
          hourly_rain.values = np.clip(hourly_rain.values, 0, None)
      else:
          hourly_rain = np.where(hourly_rain < 0, rain, hourly_rain)
          hourly_rain = np.clip(hourly_rain, 0, None)
...
def wrf_vars(ncfile, prevnc=None, cache={}):
   ...
   wspd_levels  = {f"wspd{lvl}": wrf.interplevel(wspd, heights, lvl)
                                 for lvl in wind_levels}
   wdir_levels  = {f"wdir{lvl}": wrf.interplevel(wdir, heights, lvl)
                                 for lvl in wind_levels}
   ...
   my_vars = {"uvmet": uvmet, "w": w,
              "uvmet10": uvmet10, "wspd10": wspd10, "wdir10": wdir10,
              **uvmet_levels, **wspd_levels, **wdir_levels,
...
def drjack_vars(wrf_vars):
   ...
   blwind_dir = np.mod(180 + np.degrees(np.arctan2(ublavg, vblavg)), 360)
   ...
   bltopwind = drj.calc_Wspeed(utop, vtop, name='bltopwind',
                              description='Boundary-layer-averaged wind speed')
   bltopwind_dir = np.mod(180 + np.degrees(np.arctan2(utop, vtop)), 360)
   ...
   info = {'wblmaxmin': wblmaxmin,
           ...
           'blwind': blwind, 'blwind_dir': blwind_dir,
           'uvtop': uvtop, #utop, 'vtop': vtop,
           'bltopwind': bltopwind, 'bltopwind_dir': bltopwind_dir}
