...
manage_manifest() {
    ...
    if [ -d "$plots_root" ]; then
        ...
        while IFS= read -r -d $'\0' file; do
            recent_files+=("$file")
        done < <(find "$plots_root" -type f -mmin -"$upload_minutes" \( -name "*.webp" -o -name "*.json" \) -print0)
        
        log "Encontrados ${#recent_files[@]} archivos nuevos/modificados."
...
retry_failed_uploads() {
    # No hacer nada si el FTP está deshabilitado
    if [ "${FTP_ENABLED,,}" == "false" ] || [ -z "$FTP_URL" ]; then
        return 0
    fi
    ...
    for entry in "${UPLOAD_RETRY_QUEUE[@]}"; do
        ...
        # Si ya falló un reintento en este ciclo, el resto se quedan en la cola nueva sin intentar
        if [ $failed -gt 0 ]; then
            remaining_queue+=("$entry")
            continue
        fi
        ...
        if upload_file "$file" "$dest_path"; then
            ...
            log "❌ Falló nuevamente: $(basename "$file")"
            failed=$((failed + 1))
            remaining_queue+=("$entry")
            log "⚠️ Deteniendo reintentos en este ciclo para evitar saturación."
        fi
    done
    
    # Actualizar la cola global con lo que queda pendiente
    UPLOAD_RETRY_QUEUE=("${remaining_queue[@]}")
    
    log "Resultado: $success archivos subidos, ${#UPLOAD_RETRY_QUEUE[@]} aún en cola"
    
    # Actualizar archivo persistente
    if [ ${#UPLOAD_RETRY_QUEUE[@]} -gt 0 ]; then
        printf "%s\n" "${UPLOAD_RETRY_QUEUE[@]}" > "$queue_file"
    else
        [ -f "$queue_file" ] && rm -f "$queue_file"
        log "✅ Cola persistente eliminada (todos los archivos subidos)"
    fi
    log "========================================================================"
}

# Función para procesar archivos en paralelo CON REINTENTO POR TIMEOUT
process_files() {
    ...
    for i in "${!pids[@]}"; do
        ...
        if [ $exit_code -eq 0 ]; then
            log "✅ Procesado con éxito: $(basename "$file_to_process")"
...
while true; do
    ...
    # 1. Buscar TODOS los flags wrfoutReady_d* y ordenarlos por nombre
    flags=()
    while IFS= read -r -d $'\0' f; do
        flags+=("$f")
    done < <(find "$WRF_RUN_DIR" -maxdepth 1 -name "wrfoutReady_d*" -type f -print0 2>/dev/null | sort -z)
    ...
    # Intentar subir archivos en cola de reintentos periódicamente
    retry_failed_uploads
