...
show_usage() {
    ...
    echo "Uso: [INT_SEC=n] [MPI_PROCS=n] $0 [YYYY-MM-DD-HH] [HOURS_PER_DAY] [DAYS_TO_RUN]"
    echo ""
    echo "Parámetros (posicionales):"
    echo "  1. Fecha inicio GFS (Defecto: Hoy en hora UTC más reciente 00, 06, 12 o 18)"
    echo "  2. Horas por día a simular (Defecto: 13)"
    echo "  3. Días a simular (Defecto: 3)"
    echo ""
...
download_gfs_region() {
    ...
    local day="$3"
    local hour="$4"
    local start_forecast_hour="$5"
    local end_forecast_hour="$6"
    local interval_hours="$7"
    local leftlon="$8"
    local rightlon="$9"
    local toplat="${10}"
    local bottomlat="${11}"
    ...
    echo "   Previsión: +${start_forecast_hour}h a +${end_forecast_hour}h, Intervalo: ${interval_hours}h"
    ...
    for (( i=start_forecast_hour; i<=end_forecast_hour; i+=interval_hours )); do
...
# Parámetro 1: Fecha de inicio (Opcional, Default: fecha UTC actual en hora más reciente 00, 06, 12 o 18)
if [ -n "${1-}" ]; then
    date_input="$1"
else
...
# Parámetro 2: Horas por día (Default: 13)
if [ -n "${2-}" ] && [[ "${2}" =~ ^[0-9]+$ ]]; then
    HOURS_PER_DAY="$2"
else
    HOURS_PER_DAY=13
...
# Parámetro 3: Días a simular (Default: 3)
if [ -n "${3-}" ] && [[ "${3}" =~ ^[0-9]+$ ]]; then
    DAYS_TO_RUN="$3"
else
    DAYS_TO_RUN=3
...
# El ajuste de SIM_DURATION_HOURS ahora se hace dinámicamente dentro del bucle
...
echo ""
echo "====================================="
echo " PRE-PROCESO: EJECUTANDO GEOGRID "
echo "====================================="
cd $RUN_DIR/WPS
rm -f geo_em.* geogrid.log 2>/dev/null
ln -sf "$NAMELIST_WPS" .
...
for DAY_OFFSET in $(seq 0 $MAX_DAY_OFFSET); do
    ...
    if [ $(( DURACION_DESEADA % INT_HOURS )) -ne 0 ]; then
        DURACION_AJUSTADA=$(( (DURACION_DESEADA / INT_HOURS + 1) * INT_HOURS ))
        echo "⚠️  Ajustando duración de descarga de $DURACION_DESEADA a $DURACION_AJUSTADA horas para cumplir con interval_seconds=$INT_SEC"
    else
        DURACION_AJUSTADA=$DURACION_DESEADA
    fi
    
    F_END=$(( F_START + DURACION_AJUSTADA ))
    SIM_DURATION_HOURS=$DURACION_AJUSTADA

    # Calcular Inicio Real Simulación
    START_DATE_OBJ=$(TZ=UTC date -d "$GFS_INIT_STR + $F_START hours" +"%Y-%m-%d %H")
...
END_DATE_OBJ=$(TZ=UTC date -d "$GFS_INIT_STR + $F_END hours" +"%Y-%m-%d %H")
...
# FORZAR RESTART = .FALSE. SIEMPRE PARA ARRANQUES EN FRÍO
update_param "$NAMELIST_INPUT" "restart" ".false."
...
# IMPORTANTE: No borrar geo_em.* porque se generó fuera del bucle
rm -f FILE:* PFILE:* GRIBFILE.* met_em.* *.log namelist.wps 2>/dev/null
...
# IMPORTANTE: No borrar wrfout* para no eliminar los wrfoutReady_d* de iteraciones anteriores
rm -f rsl.* wrfinput_d* met_em* *.log namelist.input 2>/dev/null
...
if grep -q "SUCCESS COMPLETE WRF" rsl.error.0000; then
    echo ""
    echo "========================"
    echo "  EJECUCIÓN COMPLETADA DÍA +${DAY_OFFSET}"
    echo "========================"
else
    echo "ADVERTENCIA: No se encontró mensaje SUCCESS en rsl.error.0000"
fi

done # Fin del bucle de 3 días
